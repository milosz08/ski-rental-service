...
public class SellerAddEquipmentToCartServlet extends AbstractWebServlet {
    private final CartEquipmentService cartEquipmentService;
    private final ValidatorBean validatorBean;

    @Inject
    public SellerAddEquipmentToCartServlet(
    ...
    protected WebServletResponse httpPostCall(WebServletRequest req) {
        ...
        final String redirect = StringUtils.trimToEmpty(req.getParameter("redirect"));
        ...
        if (validatorBean.someFieldsAreInvalid(reqDto)) {
            ...
            req.setSessionAttribute(SessionAttribute.EQ_ADD_CART_MODAL_DATA, resDto);
            throw new WebServletRedirectException("seller/complete-rent-equipments" + redirect);
        }
        final AlertTupleDto alert = new AlertTupleDto(true);
        try {
            cartEquipmentService.addEquipmentToCart(reqDto, resDto, rentData, loggedUser, equipmentId);
